# ml-data-preparation Skill Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use spml:executing-plans to implement this plan task-by-task.

**Goal:** Create the `ml-data-preparation` skill that guides dataset processing with TDD-first workflow — validate on small-scale data before running full-scale.

**Architecture:** Single skill file at `skills/ml-data-preparation/SKILL.md`. Follow existing skill conventions (frontmatter, sections, checklists). Also update `ml-brainstorming` to reference this skill when dataset processing work is identified.

---

## Task 1: Create ml-data-preparation skill

**Files:**
- Create: `skills/ml-data-preparation/SKILL.md`
...
**Step 2: Run the skill test (manual verification)**

Read through the skill and verify:
- Frontmatter `name` and `description` match conventions
- Checklist items are ordered correctly
- Code examples are syntactically valid
- Safety principles are present
- Progress display requirements are present

**Step 3: Commit**

```bash
git add skills/ml-data-preparation/SKILL.md
git commit -m "feat: add ml-data-preparation skill"
```

---

## Task 2: Update ml-brainstorming to reference ml-data-preparation

**Files:**
- Modify: `skills/ml-brainstorming/SKILL.md`

**Step 1: Add reference to ml-data-preparation**

In the `ml-brainstorming` skill, add guidance so that when dataset processing work is identified during brainstorming, the agent knows to invoke `ml-data-preparation` before or alongside the experiment plan.

Add after the "Confirming validation scope" section (after line 118), before "Exploring approaches":
...
Read the modified file and confirm the new section flows naturally between "Confirming validation scope" and "Exploring approaches".

**Step 3: Commit**

```bash
git add skills/ml-brainstorming/SKILL.md
git commit -m "feat: reference ml-data-preparation from ml-brainstorming"
```
